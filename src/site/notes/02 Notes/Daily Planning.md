...
IDEAS, QUOTES, TIPS, and REFERENCES
IDEAS
 * Consistent daily planning is fundamental for organization and productivity.
 * Time is a limited resource that must be managed effectively, similar to how Apollo 13 managed limited power.
 * Knowing your calendar appointments is the necessary first step to determine actual available task time.
 * A realistic task list must be created based on available time, not just the total number of tasks.
 * Unimportant tasks often resolve themselves if not immediately acted upon.
 * Ineitable interruptions significantly reduce available focus time for tasks.
 * Prioritizing 1-2 "must-do" tasks provides clear daily focus.
 * Using a physical, visible reminder of top priorities helps maintain focus better than hidden digital lists.
QUOTES
 * "Planning your day is one of the most critical things you could do if you want to become better organised and more productive."
 * "You also have a limited resource every single day. That resource is time."
 * "You have to follow the sequence. You can't rearrange this sequence."
 * "Most of us have no idea how to estimate how long something will take."
 * "Unimportant tasks have a habit of sorting themselves out."
 * "What are my two must do tasks for today."
 * "Be very careful about relying on a Digital to do list simply because it will be out of sight for most of the day."
 * "That really does help to get you focused on what really matters today."
TIPS
 * Plan your day consistently, including weekends.
 * Start your daily planning by checking your calendar appointments to determine available task time.
 * Create a realistic task list by removing unimportant tasks and accounting for limited time.
 * Be aware of how interruptions (emails, calls, messages) reduce actual focus time.
 * Prioritize your task list, identifying only 1-2 essential "must-do" items for the day.
 * Write your top priorities on a physical piece of paper and keep it visible to stay focused.
 * Don't rely solely on digital task managers for your most important tasks as they can be hidden from view.
 * Allocate only about five minutes for this daily planning sequence.
REFERENCES
 * Apollo 13 (movie/true story)
 ...
 * Blog posts (writing)
 * A7 notebook (tool)
 * Digital task manager (tool)
 * Franklin planner (system/tool)
 * Slack teams (tool)
 * Weekly planning matrix (project/system)
